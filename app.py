...
with tab1:
    ...
    if input_method == "Upload an Image":
        ...
        # UPDATED: Dropdown menu for easy selection
        image_id = st.selectbox("Select an Image ID to Preview:", options=list(range(1, 101)), index=9)
        
        # Fetch high-quality random images via an API
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                st.error("Image not found for this ID. Please select another.")
        except:
            st.error("Failed to load image from server. Please check your internet connection.")
            
    if image is not None:
        col1, col2 = st.columns([1, 1])
        with col1:
            # Display the visual preview instantly
            caption_text = f"Preview of Gallery Image #{image_id}" if input_method == "Select from Built-in Gallery" else "Your Uploaded Image"
            st.image(image, caption=caption_text, use_column_width=True)
            
        with col2:
            st.subheader("AI Analysis")
            st.info("👆 Preview your image on the left. When you are ready, click the button below to run the AI!")
            
            # UPDATED: Added a button so the AI only runs when the user is done previewing
            if st.button("🧠 Generate Caption", use_container_width=True):
                with st.spinner("Decoding image features... This may take a few seconds."):
